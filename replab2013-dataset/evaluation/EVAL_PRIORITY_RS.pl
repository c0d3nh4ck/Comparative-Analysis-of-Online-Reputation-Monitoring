...
while (my $line=<FICHIN>){
  ...
  if ($line=~/\w/){
    ...
    if ($priority eq "ALARM"){
      $priorityRelGold{$entity}{$id}=3;
      $entityDocs{$entity}.="|$id"; 
    }
    if ($priority eq "MILDLY_IMPORTANT"){
      $priorityRelGold{$entity}{$id}=2;
      $entityDocs{$entity}.="|$id"; 
    }
    if ($priority eq "UNIMPORTANT"){
...
while (my $line=<FICHIN>){
  ...
  if ($line=~/\w/){
    (my $entity,my $id,my $priority)=split(/\t/,$line);
    if ($priority eq "ALARM"){
      $priorityRelSys{$entity}{$id}=3;
    }
    if ($priority eq "MILDLY_IMPORTANT"){
      $priorityRelSys{$entity}{$id}=2;
    }
    if ($priority eq "UNIMPORTANT"){
